...
export default function Hero({ onCtaClick }: HeroProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-1/2 h-full bg-emerald-50/55 rounded-l-[120px] pointer-events-none -skew-x-12 origin-top-right transition-all duration-1000 hidden md:block" />
      <div className="absolute -top-10 left-1/3 w-72 h-72 bg-emerald-200/20 blur-3xl rounded-full pointer-events-none" />
      <div className="absolute top-1/2 right-10 w-96 h-96 bg-teal-200/35 blur-3xl rounded-full pointer-events-none" />

      {/* Floating 3D Geometric background objects (Decorative) */}
      <div className="absolute top-24 left-1/2 w-8 h-8 rounded bg-gradient-to-tr from-emerald-500/20 to-teal-400/20 border border-emerald-500/10 pointer-events-none animate-3d-float opacity-70" />
      <div className="absolute bottom-24 left-10 w-12 h-12 rounded bg-gradient-to-br from-teal-500/20 to-emerald-400/20 border border-teal-500/10 pointer-events-none animate-3d-isometric-float opacity-60" />
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 w-full relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        >
          ...
          <motion.div
            ...
            className="inline-block px-4 py-1.5 bg-emerald-50 text-emerald-700 text-xs font-bold tracking-widest uppercase rounded-full w-max mb-6"
          >
            <span className="flex items-center gap-1.5">
              <Cube className="w-3.5 h-3.5 text-emerald-600 animate-3d-float" />
          ...
          <motion.div variants={itemVariants}>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900 tracking-tight leading-[1.1] mb-6">
              WebNest | Leading <br />
              <span className="text-emerald-600">Website Development Company</span> <br />
          ...
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-10">
            <button
              ...
              className="px-8 py-4 bg-emerald-600 text-white font-bold rounded-xl shadow-xl shadow-emerald-200/50 hover:bg-emerald-700 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer"
            >
        ...
        >
          <div className="relative preserve-3d">
            ...
            <ThreeDCard className="shadow-3d-card bg-white rounded-3xl p-6 relative z-10 overflow-hidden" intensity={15}>
              ...
              <div className="space-y-3" style={{ transform: 'translateZ(30px)' }}>
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="col-span-2 bg-emerald-50/70 rounded-xl p-3 flex flex-col justify-between h-24 border border-emerald-100/50">
                    ...
                    <div>
                      <div className="w-3/4 h-1.5 bg-emerald-200 rounded-full mb-1" />
                      <div className="w-1/2 h-1.5 bg-emerald-200/50 rounded-full" />
                    </div>
                  </div>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-3.5 flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 rounded-full bg-white border border-slate-100 flex items-center justify-center text-emerald-600 shrink-0 shadow-sm">
                  ...
                  <div className="flex space-x-0.5 shrink-0">
                    <span className="w-1.5 h-3 bg-emerald-400 rounded-full block animate-pulse" />
                    <span className="w-1.5 h-5 bg-emerald-600 rounded-full block" />
                    <span className="w-1.5 h-2.5 bg-emerald-300 rounded-full block" />
                  </div>
                </div>
              </div>
            </ThreeDCard>
            ...
            <div className="absolute -top-6 -right-6 w-24 h-24 bg-gradient-to-tr from-emerald-600 to-teal-400 rounded-full blur-2xl opacity-20 pointer-events-none -z-10" />
          </div>
        </motion.div>
      </div>
    </section>
