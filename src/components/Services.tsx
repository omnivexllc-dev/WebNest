...
export default function Services({ onServiceSelect }: ServicesProps) {
  ...
  return (
    <section id="services" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-0 w-80 h-80 bg-blue-50/40 rounded-full blur-3xl pointer-events-none -translate-x-1/2" />
          ...
          {SERVICES_DATA.map((service) => {
            ...
            return (
              <motion.div
                ...
                className="group bg-white border border-slate-200/60 rounded-2xl p-6 hover:border-blue-600 hover:shadow-xl hover:shadow-blue-200/30 transition-all duration-300 flex flex-col justify-between shadow-sm"
              >
                <div>
                  {/* Service Icon inside themed wrapper */}
                  <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center mb-6 shrink-0 group-hover:bg-blue-600 group-hover:text-white transition-colors duration-300">
                    <IconComponent className="w-6 h-6" />
                  </div>

                  {/* Service Title */}
                  <h3 className="text-lg font-bold text-slate-900 group-hover:text-blue-600 transition-colors duration-300 mb-3 leading-snug">
                    {service.title}
                  </h3>
                ...
                <button
                  ...
                  className="w-full text-center text-xs font-bold uppercase tracking-wider py-3.5 border border-slate-200 text-slate-700 hover:text-white hover:bg-blue-600 hover:border-blue-600 rounded-xl transition-all duration-300 cursor-pointer"
                >
