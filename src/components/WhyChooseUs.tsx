...
export default function WhyChooseUs() {
  ...
  return (
    <section id="why-webnest" className="py-24 bg-white relative overflow-hidden bg-3d-blueprint">
      ...
      <div className="absolute top-1/2 right-0 w-96 h-96 bg-indigo-100/10 blur-3xl rounded-full pointer-events-none translate-x-1/2" />
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 relative z-10">
        ...
        >
          {WHY_CHOOSE_US_DATA.map((item, index) => {
            ...
            return (
              ...
              >
                <ThreeDCard 
                  className="group relative h-full bg-white border border-slate-200/60 p-8 rounded-3xl hover:border-indigo-600 transition-all duration-300 shadow-sm"
                  intensity={8}
                >
                  {/* Visual Accent Corner Bar */}
                  <div className="absolute top-0 left-0 w-2 h-0 group-hover:h-full bg-indigo-600 transition-all duration-300 rounded-l-2xl" />

                  {/* Icon wrapper */}
                  <div className="w-11 h-11 bg-slate-50 text-slate-800 group-hover:bg-indigo-50 group-hover:text-indigo-600 rounded-xl flex items-center justify-center transition-all duration-300 mb-6 shrink-0 shadow-sm border border-slate-100">
                    <IconComponent className="w-5.5 h-5.5" />
                  </div>

                  {/* Title */}
                  <h3 className="text-lg font-bold text-slate-900 mb-3 leading-snug group-hover:text-indigo-600 transition-all duration-300">
                    {item.title}
                  </h3>
        ...
        >
          ...
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-64 h-20 bg-indigo-600/25 blur-3xl rounded-full pointer-events-none" />
          ...
          <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            ...
            <div className="group/stat transition-transform duration-300 hover:scale-105">
              <span className="text-4xl md:text-5xl font-black text-indigo-400 block tracking-tight">
